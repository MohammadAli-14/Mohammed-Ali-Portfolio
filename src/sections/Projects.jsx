  ...
  return (
    <section id="projects" className="flex-center section-padding">
      <div className="w-full h-full px-4 sm:px-6 md:px-8 lg:px-10 max-w-7xl mx-auto">
        <TitleHeader
          title="🚀 My Projects"
          sub="Production-ready AI models, systems & full-stack applications"
        />

        {/* Interactive Category Filter Pills (Vercel/Linear Agency Standard) */}
        <div className="flex items-center justify-center gap-2 sm:gap-3 flex-wrap mt-8 md:mt-10">
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-medium transition-all duration-200 flex items-center gap-2 cursor-pointer active:scale-95 ${
                  isActive
                    ? "bg-white text-black font-semibold shadow-sm"
                    : "bg-zinc-900/80 hover:bg-zinc-800 border border-zinc-800 text-zinc-400 hover:text-white"
                }`}
                aria-pressed={isActive}
              >
                <span>{cat.label}</span>
                <span
                  className={`px-2 py-0.5 rounded-full text-[11px] font-mono transition-colors duration-200 ${
                    isActive
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 md:gap-6">
            {filteredProjects.map((project) => {
              ...
              return (
                <div
                  key={project.id}
                  className="project-card bg-[#09090b]/80 border border-white/10 rounded-2xl overflow-hidden hover:border-white/25 hover:shadow-[0_16px_40px_rgba(0,0,0,0.9)] transition-all duration-300 backdrop-blur-sm flex flex-col justify-between"
                >
                  {/* Top: Image & Header */}
                  <div>
                          ...
                          <button
                            ...
                            aria-label={
                              isExpanded
                                ...
                                : `Show ${project.tech.length - 4} more technologies`
                            }
                            title={
                              isExpanded
                                ? "Collapse tech stack"
                                : "Click to view all technologies"
                            }
                          >
                            {isExpanded
                              ? "− less"
                  ...
                  <div className="p-5 sm:p-6 pt-0">
                    <div className="flex gap-2 sm:gap-3 pt-3 border-t border-white/10">
                      ...
                      <a
                        href={project.code}
                        className="flex-1 py-2.5 px-3 bg-zinc-900 border border-zinc-700/80 text-zinc-200 text-center rounded-xl hover:bg-zinc-800 hover:border-zinc-500 transition-all duration-200 text-xs sm:text-sm font-medium flex items-center justify-center gap-1.5 cursor-pointer active:scale-95"
                        target="_blank"
        ...
        <div className="mt-6 sm:mt-8 text-center">
          <p className="text-zinc-500 text-sm font-mono">
            💡 Tap categories to filter • Click buttons to explore live demos
          </p>
        </div>
      </div>
    </section>
